...
const Dasboard = () => {
  ...
  const handleSubmit = async (id) => {
    try {
      ...
      if (req.status === 200) alert("Đã khóa tài khoản thành viên!");
      getAllUser();
    } catch (error) {}
  };
  ...
  const getAllUser = async () => {
    try {
      const req = await axios.get(
        `http://localhost:5000/api/getAllUserByName?status=1&username=${name}`
      );
      console.log(req);
      setUser(req.data.users);
    } catch (error) {
    ...
    <>
      ...
      <section className="content-main">
        <div className="content-header">
          <h2 className="content-title">Danh sách thành viên</h2>
          <input
            type="text"
            name="search"
            value={name}
          ...
          <div className="card-body">
            ...
            {!flag && (
              <LoadMoreUserBtn
                ...
                page={page}
                user={user}
                handleLoadMoreUser={handleLoadMoreUser}
